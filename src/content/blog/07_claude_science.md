...
That choice is the interesting part. It's the same kind of call as Claude Code shipping in the terminal instead of an IDE: a little unexpected, and a bet about where the work actually happens. I suspect it may have some advantage in setting up in an environment with highly sensitive data. Local server means the tool sits next to your data, your environment, and your compute, instead of a sandbox you upload things into. Though I have not yet been able to find any definite explanations.
...
One of the main goals of the tool is to produce papers, so naturally the artifact it wants to make is a static image. Sure it provided the code it ran to produce the structure image, but that's not the point. The demos showed an interactive HTML mode, but you can only get it if you ask specifically. Right now the structural side hands you off to other viewers to do the actual looking and beyond, and that's the part I'd most want built in.
...
There are open source skills and agents worth trying in this space, such as [open-science](https://github.com/ai4s-research/open-science), [openscience](https://github.com/synthetic-sciences/openscience), [biomini](https://biomni.stanford.edu/), [AI-Scientist](https://github.com/SakanaAI/AI-Scientist), just to name a few.

There is definitely a need for open source in this space. However upon a brief checking of these tools, I can't help but notice that most of them are built by a handful of contributors from the same group or company. The one thing I'd genuinely want is more openness here: a community building these science tools in the open, with roadmap designed by the community, instead of just publishing source code with an Apache 2.0 license. Thus, if a key member of a lab graduates, or a company decides to pivot, the open science continues to live. 

The models, skills, and agents are moving fast. Any one of them would probably get you pretty far ahead. We're not short of tools nowadays. On the contrary, there might be too many, and sometimes the real cost is the time spent choosing between them. So stick with one, and just do a trial run.
...
Fast and capable is a good starting point, and the tool is worth a look if you have the budget for it. Having an AI agent as a copilot in research is an unstoppable trend. After a session with it, the thing I keep thinking about is the question I brought in. Ask a good question and most of these tools will get you there one way or another. That part is on you, and no app does it for you.